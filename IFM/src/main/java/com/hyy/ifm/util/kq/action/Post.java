package com.hyy.ifm.util.kq.action;


import com.hyy.ifm.customer.service.CustomerService;
import com.hyy.ifm.util.kq.entity.TransInfo;
import com.hyy.ifm.util.kq.util.Base64Binrary;
import com.hyy.ifm.util.kq.util.MyX509TrustManager;
import com.hyy.ifm.util.kq.util.ParseUtil;
...
public class Post {
    ...
    * @return URL所代表远程资源的响应
	 * @throws Exception
    */
	public  HashMap sendPost(String url, String tr1XML, TransInfo transInfo,String merchantIdDel) throws Exception
	{
...
//		String jksFile = this.getClass().getResource("/kq.jks").getPath();

//		String jksFile = Thread.currentThread().getContextClassLoader().getResource("/kq.jks").getPath();
//
//		String jksFile = Post.class.getResource("/kqX.jks").getPath();
		//获取证书路径
//		File certFile = new File(jksFile);  //测试证书，生产环境需要替换
		//访问Java密钥库，JKS是keytool创建的Java密钥库，保存密钥。

		InputStream in = Post.class.getClassLoader().getResourceAsStream("kq.jks");
		...
		try
		{
			...
			String authString = merchantIdDel+ ":" + "vpos123";//测试帐号，生产环境需要替换生产商户编号812451145110002
