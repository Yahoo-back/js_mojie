...
(function ($) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, columns) {
            ...
            $.each(columns, function (j, column) {
                ...
                html.push('</div>');
                var inputText = "";
                if (column.fuzzy_condition != undefined ) {
                	//that.header.stateField =__addSearchFieldToTitle(column);
                	inputText = __addSearchFieldToTitle(column);
                }
	...
	function __addSearchFieldToTitle(column) {
		...
		var condition_item = "";
		var fuzzy = column.fuzzy_condition;
		var fileds = "";
		var f ;
		...
		case "text_inteval":
			condition_item = "<input " + __idName(column, "_FROM")
					+ " type='text' style='width:40%;'class='form-control' /> - <input "
					+ __idName(column, "_TO")
					+ " type='text' style='width:40%;' class='form-control' />";
			break;
		case "select":
			var idName = "";
			var _id = column.field + "_cnd" ;
			if(f.length > 1){
				condition_item = $("#"+_id).fuzzySelect(f[1]);
			}else{
				condition_item = $("#"+_id).fuzzySelect(_id);
			}
			break;
		case "date":
			condition_item = "<input "
					+ __idName(column)
					+ " onclick='WdatePicker()' class='form-control' readonly='readonly' type='text' style='width:100%;' />";
			break;
		case "exact_date":
			condition_item = "<input "
				+ __idName(column)
		...
		case "date_inteval":
			condition_item = "<input "
					...
					+ " onclick='WdatePicker()' class='form-control' readonly='readonly' type='text' style='width:45%;' /> - <input "
					+ __idName(column, "_TO")
					+ " onclick='WdatePicker()' class='form-control' readonly='readonly' type='text' style='width:45%;' />";
			break;
		case "ref":
		...
		case "select":
			...
			break;
		default:
			if (typeof column.fuzzy_condition === 'function') {
    ...
    $.extend({  
        bootstrapFuzzy:function(bootstapid){  
        	var bt = $("#"+bootstapid).bootstrapTable("getOptions");
        	var columns = bt.columns;
        	if(columns!=undefined){
        		columns = columns[0];
        		var arr = {};
        		for(var i in columns){
        			if(typeof(columns[i].field) != undefined && columns[i].field != 0){
        				arr[columns[i].field+"_cnd"] = $("#"+columns[i].field+"_cnd").val();
        			}
        		}
        		var param = {};
        		param.cnds = encodeURIComponent(JSON.stringify(arr))
        		 $("#"+bootstapid).bootstrapTable('refresh', {
        	         url: bt.url, //重设数据来源
        	         query: param//传到后台的参数
        	     });
        	}
