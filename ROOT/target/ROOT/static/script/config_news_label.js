$(function() {
    ...
    var cust_tb = $.et.commonQuery('#cust_tb',{
        ...
        fit:true,
        queryParams:{"cnds":"{'type' : '1'}"},
        fitColumns:true,
        singleSelect:false,
        columns:[[
            {field:"name",title:"标签名称",width:100,sortable:true,headalign:"center",fuzzy_condition:"text"},
            ...
            {field:"BIZ_OPT",title:"操作",width:100,sortable:true,headalign:"center",fuzzy_condition:function(){
                ...
                value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:editLabel('"+row.id+"');\" style=\"color: #FF4500;\">编辑</a>";
                if(row.status == "0") {
                    value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:underLabel('"+row.id+"');\" style=\"color: #FF4500;\">关闭</a>";
                } else {
                    value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:putLabel('"+row.id+"');\" style=\"color: #FF4500;\">启用</a>";
                }
                value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:deleteLabel('"+row.id+"');\" style=\"color: #FF4500;\">删除</a>";
                return value;
            }}
        ]],
...
function underLabel(id) {
    if(id == "" || id == null || id == undefined){
        return;
    }

    $.messager.confirm('提示', '确定要关闭该标签吗？关闭后文章不能关联此标签，并且前端不展示', function(r){
        if (r){
            ajaxRequest("updateClassify",{"id":id, "status" : "1", "type" : "2"}, function(data){
                $.messager.alert('提示',data.resultNode,'info');
                if(data.resultNode == "success"){
                    $("#cust_tb").datagrid('reload');
                }
...
function putLabel(id) {
    if(id == "" || id == null || id == undefined){
        return;
    }

    $.messager.confirm('提示', '确定要开启该标签吗？', function(r){
        if (r){
            ajaxRequest("updateClassify",{"id":id, "status" : "0", "type" : "2"}, function(data){
                $.messager.alert('提示',data.resultNode,'info');
                if(data.resultNode == "success"){
                    $("#cust_tb").datagrid('reload');
                }
...
function deleteLabel(id) {
    ...
    $.messager.confirm('提示', '确定要删除该标签吗？删除后文章不能关联此标签，并且前端不展示', function(r){
        if (r){
            ajaxRequest("updateClassify",{"id":id, "status" : "2", "type" : "2"}, function(data){
                $.messager.alert('提示',data.resultNode,'info');
                if(data.resultNode == "success"){
                    $("#cust_tb").datagrid('reload');
                }
    ...
    $.messager.confirm('提示', '确定导出吗？', function(r){
        if (r){
            ...
            ajaxRequest("exportNewsTypeList",condition, function(data){
                if(data.resultNode == "success"){
                    ExporterExcel($("#ex_cust_tb").datagrid("options").columns,'资讯标签信息','详情',data.rows);
                }else{
                    $.messager.alert('提示',data.resultNode,'info');
                }
                $(".load1").hide();
