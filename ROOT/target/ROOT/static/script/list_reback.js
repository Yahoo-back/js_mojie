    ...
    var cust_tb = $.et.commonQuery('#cust_tb',{
        ...
        columns:[[
            ...
            {field:"BIZ_OPT",title:"操作",width:100,sortable:true,headalign:"center",fuzzy_condition:function(){
                    ...
                    if (row.refundstatus == "1" || row.refundstatus == "4") {
                        value += "&nbsp;<a onclick=\"javascript:reback('" + row.id + "','"+ row.refund_amt + "');\"  style=\"color: #FF4500;\">退款</a>";
                    }
                    return value;
                }}
            // {field:"BIZ_OPT",title:"操作",width:100,sortable:true,headalign:"center",fuzzy_condition:function(){
...
function views(id){
    var url = ctx+"/complaint/form_reback?id="+id+"&view=1";
...
function qryRefundList(id, view) {
    ...
    $.messager.prompt({
        ...
        fn: function (r) {
            if (r) {
                ...
                if(!exp.test(r)) {
                    $.messager.alert('提示', "请输入退款金额!<br>退款金额应小于" + refundAmt, 'info', function () {
                        reback(id,refundAmt)
                    });
                    return;
                }
                if(r > refundAmt) {
                    $.messager.alert('提示', '退款金额应小于待退款金额!', 'info', function () {
                        reback(id,refundAmt)
                    });
                    return;
                }
                ajaxLoading();
                ajaxRequest("thirdPay",{"id":id,"money": r,"status" : "1","type": "0"},
                    function(data){
