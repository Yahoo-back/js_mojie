function pic(id,obj){
	...
	$.each(obj,function(i){
		...
		surl = (surl.indexOf("http://") >= 0 || surl.indexOf("https://") >= 0) ? surl : st.webSite+obj[i].file_uri;
...
function picView(id,obj){
	...
	$.each(obj,function(i){
		...
		html += "<img class='images' src='"+(surl)+"' style='width:150px;height:100px' /></a></td>";
		paintTab(id,html);
	});
	/*var objs=[document.getElementsByName("file_uri")];
	$.each(objs,function(){
