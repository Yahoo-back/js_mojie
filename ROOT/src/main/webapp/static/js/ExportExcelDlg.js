...
var JSXmlExcel = {
    ...
    BulidXml: function (dataOpts) {
      ...
      '" x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="150" ss:DefaultRowHeight="24">' + titleXml + headerXml + rowxml + footxml +
        '</Table></Worksheet>';
    ...
    EncodeValue: function (content) {
        var code = { '<': '&lt;', '>': '&gt;', '&': '&amp;', '\'': '&apos;', '"': '&quot;' };
        var encodeC = "";
        //encodeC = content.replace(/'/g, '&apos;').replace(/"/g, '&quot;')/*.replace(/&(?![a-zA-Z]{1,10};)/, "&amp;")*/;
        return null == content || undefined == content ? "" : content;
    }
};
; (function ($) {
