$(function() {
	 var cust_tb = $.et.commonQuery('#cust_tb',{
		url:st.apiPath+'qryUserList',
		 fit:true,
		fitColumns:true,
		singleSelect:false,
		columns:[[
			{field:"user_code",title:"登录账号 ",width:100,sortable:true,headalign:"center",fuzzy_condition:"text"},
			{field:"user_name",title:"员工姓名 ",width:100,sortable:true,headalign:"center",fuzzy_condition:"text"},
			...
			{field:"BIZ_OPT",title:"操作",width:150,sortable:true,headalign:"center",fuzzy_condition:function(){
				...
				if("0" == row.status || "4" == row.status ){
					...
					value += '&nbsp;<a href="javascript:void(0);" onclick="javascript:add_user('+row.lgn_id+', '+row.status+');" style="color: #FF4500;">修改</a>';
...
function views(id){
	var url = ctx+"/users/form_user?id="+id;
	$("#add_user").dialog({
		title : "查看信息",  href:url,
		fit:true,
		/*
...
function delete_user(id,status){
	$.messager.confirm('提示', '确定要删除该员工吗？', function(r){
		if (r){
...
function saveUser(){
	...
	$.messager.confirm('提示', '确定保存吗？', function(r){
        if (r){
        	ajaxRequest("saveUser",par, function(data){
        		$.messager.alert('提示',data.resultNode,'info');
        		if(data.resultNode == "success"){
        			$("#add_user").dialog({ closed : true });
