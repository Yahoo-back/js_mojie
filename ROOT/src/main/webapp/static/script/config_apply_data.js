$(function() {
    $("#data_type_cnd").val("APPLY_DATA");
    var cust_tb = $.et.commonQuery('#cust_tb',{
        url:st.apiPath+'qryDictConfigList',
        fit:true,
        queryParams:{"cnds":"{'data_type' : 'APPLY_DATA'}"},
        fitColumns:true,
        singleSelect:false,
        columns:[[
            {field:"item_value",title:"资料名称",width:100,sortable:true,headalign:"center",fuzzy_condition:"text"},
            ...
            {field:"BIZ_OPT",title:"操作",width:100,sortable:true,headalign:"center",fuzzy_condition:function(){
                ...
                value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:editApplyData('"+row.id+"');\" style=\"color: #FF4500;\">编辑</a>";
                if(row.is_use == "1") {
                    value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:underApplyData('"+row.id+"');\" style=\"color: #FF4500;\">下架</a>";
                } else {
                    value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:putApplyData('"+row.id+"');\" style=\"color: #FF4500;\">上架</a>";
                }
                value += "&nbsp;<a href=\"javascript:void(0);\" onclick=\"javascript:deleteApplyData('"+row.id+"');\" style=\"color: #FF4500;\">删除</a>";
                return value;
            }}
        ]],
...
function saveApplyData() {
    ...
    par.id = $("#id").val();
    par.data_type = "APPLY_DATA";

    $.messager.confirm('提示', '确定保存该申请资料吗？', function(r){
        if (r){
...
function underApplyData(id) {
    if(id == "" || id == null || id == undefined){
        return;
    }

    $.messager.confirm('提示', '确定要下架该申请资料吗？', function(r){
        if (r){
...
function deleteApplyData(id) {
    ...
    $.messager.confirm('提示', '确定要删除该申请资料吗？', function(r){
        if (r){
    ...
    $.messager.confirm('提示', '确定导出吗？', function(r){
        if (r){
            ...
            ajaxRequest("exportDictConfigList",condition, function(data){
                if(data.resultNode == "success"){
                    ExporterExcel($("#ex_cust_tb").datagrid("options").columns,'申请资料信息','详情',data.rows);
                }else{
                    $.messager.alert('提示',data.resultNode,'info');
                }
                $(".load1").hide();
